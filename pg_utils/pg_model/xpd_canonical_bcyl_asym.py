...
Expansion for the canonical variables [extended with boundary terms]
    for equatorially anti-symmetric ("dipolar" symmetry) b field
...
identifier = "expand_eigen_canonical_ext-bound_anti-sym"
...
bases_s_expression = base.LabeledCollection(
    ...
    # Boundary fields
    B_pp = expansion.jacobi_2_side(1, Abs(m + 1), S.Half, Abs(m + 1) - S.Half),
    B_pm = expansion.jacobi_2_side(1, Abs(m - 1), S.Half, Abs(m - 1) - S.Half),
    Bz_p = expansion.jacobi_2_side(0, Abs(m), -S.Half, Abs(m) - S.Half),
    B_mp = expansion.jacobi_2_side(1, Abs(m + 1), S.Half, Abs(m + 1) - S.Half),
    B_mm = expansion.jacobi_2_side(1, Abs(m - 1), S.Half, Abs(m - 1) - S.Half),
    Bz_m = expansion.jacobi_2_side(0, Abs(m), -S.Half, Abs(m) - S.Half),
)
