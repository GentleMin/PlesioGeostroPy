...
Expansion for the transformed variables [extended with boundary terms]
    for symmetric ("quadrupolar" symmetry) b field
...
identifier = "expand_conjugate_eigen_ext-bound_sym"
...
bases_s_expression = base.LabeledCollection(
    ...
    # Boundary fields
    B_pp = expansion.orth_pref_jacobi(0, Abs(m + 1)),
    B_pm = expansion.orth_pref_jacobi(0, Abs(m - 1)),
    Bz_p = expansion.orth_pref_jacobi(1, Abs(m)),
    B_mp = expansion.orth_pref_jacobi(0, Abs(m + 1)),
    B_mm = expansion.orth_pref_jacobi(0, Abs(m - 1)),
    Bz_m = expansion.orth_pref_jacobi(1, Abs(m))
)
